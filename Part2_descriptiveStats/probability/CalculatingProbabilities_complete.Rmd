...
disp(paste0('the probability of low power is:  ',
            round(lowProb,2),
               '\nthe probability of med power is:  ',
            round(medProb,2), 
               '\nthe probability of high power is: ',
            round(highProb,2)))
...
sampDf <- data.frame( 
          low = c(sum(sampleVals<cutOffLow), lowProb*sampN),
          med = c(sum(sampleVals>=cutOffLow & 
                        sampleVals <cutOffHigh),
                  medProb*sampN), 
          high= c(sum(sampleVals>=cutOffHigh),
                  highProb*sampN),
