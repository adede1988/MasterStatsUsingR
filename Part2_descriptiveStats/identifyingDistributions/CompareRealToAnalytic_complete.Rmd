...
x <- df$pow_delta_O
analyticDists <- data.frame('norm' = 
                     dnorm(x, mean = EEGsum$meanVal, 
                              sd = EEGsum$sdVal),
                            'poisson' = 
                     dpois(round(x), lambda = EEGsum$meanVal), 
                            'logNorm' = 
                     dlnorm(x, meanlog = EEGsum$logMean, 
                              sdlog = EEGsum$logSD), 
                     'xVal' = x)
...
df %>% ggplot(aes(x = pow_delta_O, color = dist)) + 
  ...
  geom_line(data = analyticDists, 
            aes(x = xVal, y = density), linewidth = 2) + 
  brightCol +  myTheme + 
  ggtitle("EEG data") 
...
plotAgainstDists <- function(df, 
  ...
  df$dist = "empirical data"
  df %>% ggplot(aes_string(x = varOfInterest, 
                           color = 'dist')) + 
          geom_freqpoly(aes(y = after_stat(density)), 
                        linewidth = 2, bins = binCount) +
          geom_line(data = analyticDists, aes(x = xVal, 
                                              y = density), 
                        linewidth = 2) + 
          brightCol +  myTheme + 
          ggtitle(myTitle) 
...
#do I get more emails on weekdays or weekends? 
...
sp500Sum <- df %>% summarize(setName = 'sp500', 
                             ...
                             sdVal = sd(Change), 
                             logMean =
                      mean(log(Change-min(Change)+.01)),
                             logSD =
                      sd(log(Change-min(Change)+.01)), 
