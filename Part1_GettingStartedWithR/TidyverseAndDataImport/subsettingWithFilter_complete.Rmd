...
## Combining filter with %in% for subsetting

```{r more subsetting}

#what if we want to analyze only a target subset of countries

targCountries = c('Tunisia', 'Spain', 'Panama', 'Myanmar',
                  'Morocco', 'Jamaica', 'French Polynesia',
                  'Chile')
#it won't work to just check for equality: 
# cw %>% filter(country == targCountries)

#need to use the %in% opperator: 
cw %>% filter(country %in% targCountries)
